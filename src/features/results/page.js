...
export async function resultsPage({ publicView = false } = {}) {
  ...
  const actions = publicView ? '' : `<button class="btn btn-secondary" type="button" data-export-results><i data-lucide="Download"></i>Ekspor CSV</button><button class="btn btn-primary" type="button" data-publish ${incomplete.length || event.status !== 'scoring_open' ? 'disabled' : ''}><i data-lucide="Trophy"></i>Buat snapshot & terbitkan</button>`;
